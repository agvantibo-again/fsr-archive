...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) {
  #define MEM_PANIC_ON(arg) do { if (arg == MEM_ERROR) \
      {Stack_destroy(&ic); Queue_clear(&ostream); Queue_clear(&tr1); \
      Queue_clear(&tr1_s); return 2;}} while (0)
  ...
  enum RetCode ret;
  ret = Stack_init(&ic); MEM_PANIC_ON(ret);
  scanf("%d", &n);
  if (n <= 0) {
    return 1;
  }
  ...
  for (int i = 0; i < n; ++i) {
    scanf("%d", cars + i);
    ret = Queue_push(&tr1, *(cars + i)); MEM_PANIC_ON(ret);
  }

  qsort(cars, (size_t) n, sizeof(int), cmp);

  for (int i = 0; i < n; ++i) {
    ret = Queue_push(&tr1_s, *(cars + i)); MEM_PANIC_ON(ret);
  }
  //   bool solveable = true;
  bool salvageable = false;
  enum RetCode has_tr1;
  while (
    has_tr1 = Queue_front(&tr1, &buf),
    has_tr1 == OK || salvageable
  ) {
    Queue_front(&tr1_s, &tgt);
    Queue_front(&tr1, &frc);
    ret = Stack_back(&ic, &buf);
    if (ret == OK && buf == tgt) {
      ret = Queue_push(&ostream, 2); MEM_PANIC_ON(ret);
      Stack_pop(&ic, &buf);
      Queue_pop(&tr1_s, &buf);
    } else if (frc == tgt) {
      ret = Queue_push(&ostream, 1); MEM_PANIC_ON(ret);
      ret = Queue_push(&ostream, 2); MEM_PANIC_ON(ret);
      Queue_pop(&tr1, &buf);
      Queue_pop(&tr1_s, &buf);
    } else {
      ret = Queue_push(&ostream, 1); MEM_PANIC_ON(ret);
      ret = Stack_push(&ic, frc); MEM_PANIC_ON(ret);
      Queue_pop(&tr1, &buf);
    }
  }
  salvageable = true;
