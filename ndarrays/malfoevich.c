...
struct pair index_of_2da(int const  n, int const m, char const scan[n][m], char const f) {
  int r_0 = 0;
  int c_0 = 0;
  bool found = false;
  ...
  --r_0; --c_0; // since we haven't broken out, both got incremented by 1
...
bool fill_next_rect(int const n, int const m, char scan[n][m], char const f) {
  ...
  int r = c_0.r;
  int c = c_0.c;
  for (r = c_0.r; r < n && scan[r][c] == '#'; ++r) { // short-circuits because of the former
...
static bool generate_b(int const n, int const m, char scan[n][m]) {
  ...
  if (count_in_2da(n, m, scan, 'a') <= 1) { // return early if we cant produce rect b
    return false;
  }
  if (ca_0.c + 1 == m || scan[ca_0.r][ca_0.c] != 'a') { // edge case: a is 1 wide
    scan[ca_0.r][ca_0.c] = 'b'; // b is 1x1 at the beginning of a, a is an 1xk strip
    return true;
  } 
  for (int r = ca_0.r; r < n; ++r) {
    if (scan[r][ca_0.c] == 'a') {scan[r][ca_0.c] = 'b';}
  }
  return true;
}
